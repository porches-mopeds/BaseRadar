...
const NETWORKS: Record<"mainnet" | "sepolia", Network> = {
  mainnet: {
    chain: base,
    chainId: 8453,
...
let active: Network = NETWORKS.sepolia;

const APP = {
  name: "Base Radar (Built for Base)",
...
async function readBlockSignal() {
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain validation, and continuous read-only onchain radar.";
  ...
  btnSignal.textContent = "Block Signal";
  ...
  btnSignal.onclick = async () => {
    try {
      render(["Reading block signal…"]);
      const s = await readBlockSignal();
      render([
        "Block Signal",
        `Network: ${active.label}`,
        `Block: ${s.number}`,
  ...
  [btnConnect, btnToggle, btnSignal, addrInput, btnAddr].forEach((el) => {
